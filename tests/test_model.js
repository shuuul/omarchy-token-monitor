...
assert.strictEqual(model.barText(rows, false, "codex"), "Codex 59%")
assert.strictEqual(model.barIconId(rows, false, "codex"), "codex")
assert.strictEqual(model.barText(rows, false, "kimi"), "Kimi Code")
assert.strictEqual(model.barIconId(rows, false, "kimi"), "kimi")
assert.ok(model.barSlotLabels(rows, false).includes("Codex 59%"))
assert.ok(model.barSlotLabels(rows, false).includes("Cursor 91%"))
assert.ok(model.barSlotLabels(rows, false).includes("Kimi Code"))
assert.strictEqual(model.longestBarLabel(rows, false), "Cursor 91%")
