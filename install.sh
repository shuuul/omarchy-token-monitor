...
plugin_id="shuuul.token-monitor"
...
if ! command -v python3 >/dev/null 2>&1; then
  printf '%s\n' 'Note: python3 is not on PATH. The panel cannot collect usage without it.' >&2
fi
...
printf '%s\n' 'Registering Token Monitor in the bar…'
omarchy-shell shell rescanPlugins
omarchy plugin enable "$plugin_id"

printf 'Token Monitor installed for development at %s\n' "$install_path"
