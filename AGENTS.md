...
- The bar shows the selected provider icon plus two remaining meters: weekly on top, session underneath. Do not put the provider name or percent text in the bar slot. Grok's single credits window is weekly, not session. Cursor uses Cursor Models on top and Third-Party underneath.
...
- Panel settings live behind the hero gear: refresh interval (default 5 minutes), Chrome/Chromium cookies, and per-provider enablement. Persist with `bar.shell.updateEntryInline`.
