...
- Panel settings live behind the hero gear: refresh interval (default 5 minutes), refresh on open (default off), show remaining vs used (default remaining), Chrome/Chromium cookies, and per-provider enablement. Persist with `bar.shell.updateEntryInline`. Settings rows can be dragged to set `providerOrder`; the main provider switch uses that same order. Opening the panel fetches every provider only when refresh on open is enabled.
